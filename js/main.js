...
var positions = [],
  build_toc = function () {
    ...
    $('.post-content').find('h2').each(function (i) {
      ...
      output += '<li><a  href="#title_' + i + '" class="title toc-title_' + i + '">' + svg + $(this).text() + '</a> <ol>';
      $(this).nextUntil('h2', 'h3').each(function (j) {

        $(this).attr('id', this.innerHTML.hashCode().replace("-",""));
        console.log(this);
        output += '<li><a href="#'+this.innerHTML.hashCode().replace("-","")+'" class="toc-subtitle_' + i + '_' + j + '">' + $(this).text() + '</a>';
        i = 4
      });
      output += '</ol></li>'
    });
